...
const GettingStarted = ({}: GettingStartedProps) => {
  ...
  useEffect(() => {
    ...
    const groupIdFromUrl =
      navigation.find((group) => url.includes(group.id))?.id || "g2";
    setOpenGroupId(groupIdFromUrl);
  }, []);

  const getColorClass = (url: string) => {
    if (url.includes("client-side-object-storage-encryption")) {
      return "text-[#00BBDD]"; 
    }
    return "text-n-2"; 
  };
    ...
    <Section>
      <div className="container md:py-10 lg:pt-16 xl:pt-20">
        <Heading
          ...
          titleLarge="Client Side Object Storage Encryption for S3"
          textLarge="Encrypt your files on the client side before sending them to your S3 buckets with just 3 blocks of code"
        />
