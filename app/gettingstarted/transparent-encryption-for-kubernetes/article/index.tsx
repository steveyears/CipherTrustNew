...
const GettingStarted = ({}: GettingStartedProps) => {
  ...
  useEffect(() => {
    ...
    const groupIdFromUrl =
      navigation.find((group) => url.includes(group.id))?.id || "g1";
    setOpenGroupId(groupIdFromUrl);
  }, []);

  const getColorClass = (url: string) => {
    if (url.includes("transparent-encryption-for-kubernetes")) {
      return "text-[#00BBDD]"; 
    }
    return "text-n-2"; 
  };
    ...
    <Section>
      <div className="container md:py-10 lg:pt-16 xl:pt-20">
        <Heading
          ...
          titleLarge="Transparent Encryption for Kubernetes"
          textLarge="Get started with running CipherTrust's Transparent Encryption connector with Kubernetes"
        />
