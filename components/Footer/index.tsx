...
const Footer = ({}: FooterProps) => (
    ...
    >
        ...
        <div className="lg:flex lg:items-center lg:justify-between">
            <div className="hidden caption text-n-2 lg:block">
               Copyright © {new Date().getFullYear()} THALES. All rights reserved.
            </div>
            <div className="flex justify-center -mx-4">
                {socials.map((item) => (
                    <a
                        className="flex items-center justify-center w-10 h-10 mx-4 bg-n-8 rounded-full transition-colors hover:bg-n-5"
                        href={item.url}
                        target="_blank"
                        key={item.id}
                    >
